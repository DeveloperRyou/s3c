  ...
  return (
    <Row className={props.data.className+" mb-5"}>
      <Col lg={7} md={12} className="content_en">
        {props.data.content_en.map((content, index) =>
          <Row key={index} className="mb-3">
            <Col lg={1} md={2} className="m-auto offset-lg-1 offset-1">
              <p className="common-p sc3-color h5 m-auto">{content.percent}</p>
            </Col>
            <Col lg={10} md={9} className="m-auto">
              <p className="common-p m-auto">{content.content}</p>
            </Col>
          </Row>
        )}
      </Col>
      <Col lg={7} md={12} className="content_kr d-none">
        {props.data.content_kr.map((content, index) =>
          <Row key={index} className="mb-3">
            <Col lg={1} md={2} className="m-auto offset-lg-1 offset-1">
              <p className="common-p sc3-color h5 m-auto">{content.percent}</p>
            </Col>
